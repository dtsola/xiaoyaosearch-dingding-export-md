...
```bash
# 核心依赖
pnpm add commander inquirer cli-progress axios cheerio tough-cookie
pnpm add playwright pino pino-pretty cosmiconfig glob p-limit

# 开发依赖
pnpm add -D typescript @types/node @types/inquirer @types/cli-progress
pnpm add -D @types/tough-cookie tsup tsx
...
**验证：** `pnpm dev export -d <docId>` 能成功导出文档
