...
- 不考虑不同知识库导出到同一目录的情况
- 每次导出覆盖/创建新的 `xiaoyaosearch-dingding-manifest.xyjson`
- 保持代码简洁，最小化改动
...
- **文件名**: `xiaoyaosearch-dingding-manifest.xyjson`
- **位置**: 导出目录的根目录
- **编码**: UTF-8
...
```
D:\我的文档\钉钉导出\
├── xiaoyaosearch-dingding-manifest.xyjson  ← 元数据文件
├── 工作文档\
│   ├── 项目报告.docx
│   └── 会议纪要.docx
└── 产品资料\
...
const manifest = JSON.parse(
    fs.readFileSync('xiaoyaosearch-dingding-manifest.xyjson', 'utf-8')
);
```
